...
export default class App extends React.Component {
  ...
  editItem = (item, count) => {
    ...
    let existentItem = cartCopy.find((cartItem) => cartItem.id === item.id);
    ...
    if (!existentItem) {
      ...
      cartCopy = cartCopy.map((cartItem) => {
        if (cartItem.id === existentItem.id) {
          return existentItem;
        } else {
          return cartItem;
        }
    ...
    return (
      <BrowserRouter>
        <>
          ...
          />
          <Switch>
            {/* <Route path="/" element={<Navigate replace to="/woman" />} /> */}
            <Route path="/cart">
              <Cart
                selectedCurrency={this.state.selectedCurrency}
                handleChangeCurrency={this.handleChangeCurrency}
            ...
            <Route path="/">
              <MainWithRouter selectedCurrency={this.state.selectedCurrency} />
            </Route>
          </Switch>
        </>
      </BrowserRouter>
